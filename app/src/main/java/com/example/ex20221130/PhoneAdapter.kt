...
class PhoneAdapter(val context: Context, val layout: Int, val data: ArrayList<PhoneVO>) : BaseAdapter() {
    ...
    override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {
        ...
        // 메인액티비티 화면 정보를 가지고 있는 context 활용
        // Inflate : 코드로 있는 xml파일을 눈에 보이는 뷰로 바꿔주는 작업
        ...
        // p0 : 항목의 번호 (position)
        // p1 : 이전에 만들어진 View (xml을 눈에 보이는 형태로 바꾼 거)
        ...
        if(view == null) {
            ...
            btnCall?.setOnClickListener {
                ...
                // (인텐트 쓸 때 스타트액티비티를 해줘야하는데 액티비티가 아니라
                // 스타트액티비티를 쓸 수 없으므로)
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                // Activity의 힘(:context)을 빌려서 Start할 예정
